...
Use a Hugging Face write token scoped only as broadly as required. Set it in the current PowerShell process; do not paste it into source, logs, screenshots, shell history files, or the publication receipt.
...
Preflight verifies authenticated namespace ownership and confirms that the target Space does not already exist. It performs no remote mutation.
...
The receipt contains the remote revision, public application URL, local Git SHA, candidate manifest hash, aggregate candidate-tree hash, all 35 file hashes, and verification outcomes. It contains no credential material.
