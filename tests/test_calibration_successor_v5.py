"""Tests for the V5 quarantined final mixed-reliability boundary."""
...
_MIXED_CASE_IDS = tuple(f"CSV5-{number:03d}" for number in range(228, 237))
...
def test_final_mixed_root_requires_exactly_thirty_six_heldout_case_pairs(
    tmp_path: Path,
) -> None:
...
def test_final_mixed_root_rejects_adversarial_case_contamination(tmp_path: Path) -> None:
    root = _copy_fixture_root(tmp_path)
    source = root / "final_evaluation" / "inputs" / "cases" / "CSV5-236.json"
    target = root / "final_evaluation" / "inputs" / "cases" / "CSV5-301.json"
...
def test_registry_retains_later_final_and_adversarial_reservations_as_quarantined() -> None:
    ...
    later_final_families = [
        ...
        and family not in (curve_family, position_family, workload_family, mixed_family)
    ]
    ...
    assert mixed_family.reserved_case_ids == _MIXED_CASE_IDS
    assert mixed_family.authoring_status == "final_mixed_reliability_contrast_authored"
    assert not later_final_families
    assert all(
