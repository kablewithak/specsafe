"""Tests for the V5 quarantined final position-spread boundary."""
...
from specsafe.traces.calibration_successor_v5 import (
    ...
    assert_calibration_successor_v5_final_position_spread_fixture_root,
...
def test_loads_the_v5_registry_only_through_final_position_spread_boundary() -> None:
    registry = load_calibration_successor_v5_scenario_family_registry(
        _REGISTRY_PATH,
        allow_final_position_spread_assets=True,
    )

    assert registry.registry_status == "final_position_spread_authored"
    ...
    assert registry.next_authorized_artifact == "v5-final-evaluation-workload-variation-fixtures"
...
def test_final_position_root_requires_exactly_eighteen_heldout_case_pairs(tmp_path: Path) -> None:
    root = _copy_fixture_root(tmp_path)
    (root / "final_evaluation" / "inputs" / "cases" / "CSV5-218.json").unlink()
...
def test_final_position_root_rejects_later_final_case_contamination(tmp_path: Path) -> None:
    root = _copy_fixture_root(tmp_path)
    source = root / "final_evaluation" / "inputs" / "cases" / "CSV5-218.json"
    target = root / "final_evaluation" / "inputs" / "cases" / "CSV5-219.json"
...
def test_registry_rejects_final_position_status_without_manifest_provenance() -> None:
...
def test_registry_retains_later_final_and_adversarial_reservations_as_quarantined() -> None:
    ...
    later_final_families = [
        ...
        and family not in (curve_family, position_family)
    ]
    ...
    assert position_family.is_final_evaluation_quarantined is True
    assert all(
