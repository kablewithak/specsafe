...
def test_position_spread_inventory_is_exact_and_physically_separated() -> None:
    ...
    input_names = {
        path.name for path in (_FIXTURE_ROOT / "inputs" / "cases").iterdir()
    }
    outcome_names = {
    ...
    assert len(input_names) == 24
    assert len(outcome_names) == 24
...
def test_position_spread_confidence_declines_with_position_and_retains_outcome_mix() -> None:
    ...
    for case_id in _CASE_IDS:
        ...
        confidences = [
            ...
            for context in runtime_payload["contexts"]
        ]
        ...
        for outcome in replay_case.expected_outcomes.outcomes:
            accepted_by_position[outcome.block_position_index].append(
