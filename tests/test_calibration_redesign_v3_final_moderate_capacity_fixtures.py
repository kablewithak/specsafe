...
CASE_IDS = tuple(f"CRV3-{number:03d}" for number in range(207, 213))


def test_final_moderate_capacity_cases_are_quarantined_balanced_and_unscored() -> None:
...
def test_final_moderate_capacity_inputs_use_moderate_request_pressure() -> None:
    for case_id in CASE_IDS:
        replay_case = load_calibration_redesign_v3_final_evaluation_replay_case(
            FIXTURE_ROOT, case_id
        )
        assert all(
            4 <= context.capacity_snapshot.active_request_count <= 6
            for context in replay_case.runtime_input.contexts
        )
        assert [
