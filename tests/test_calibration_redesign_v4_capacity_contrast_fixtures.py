"""Calibration-only diagnostic tests for V4 capacity-contrast fixtures."""
...
_CASE_IDS = tuple(f"CRV4-{number:03d}" for number in range(137, 149))


def test_capacity_contrast_inventory_is_present_in_both_physical_asset_trees() -> None:
...
def test_capacity_contrast_covers_profiles_workloads_and_positions() -> None:
    replay_cases = tuple(
        load_calibration_redesign_v4_replay_case(_FIXTURE_ROOT, case_id) for case_id in _CASE_IDS
    )

    workloads = Counter(
        replay_case.runtime_input.contexts[0].workload_type.value for replay_case in replay_cases
    )
    positions = Counter(
    ...
    assert profile_case_counts == Counter(
        {
            "v4-calibration-capacity-light": 3,
            "v4-calibration-capacity-moderate": 3,
            "v4-calibration-capacity-saturated": 3,
            "v4-calibration-capacity-jagged": 3,
        }
    )
...
def test_capacity_contrast_runtime_assets_are_label_free_and_preserve_contrast() -> None:
    ...
    for case_id in _CASE_IDS:
        ...
        if contexts[0]["capacity_snapshot"]["profile_id"].endswith("jagged"):
            jagged_snapshots.append(
                tuple(context["capacity_snapshot"]["active_request_count"] for context in contexts)
            )

        assert payload["scenario_family_id"] == "CRV4-CAL-CAPACITY-CONTRAST"
    ...
    assert len(context_shapes) >= 4
    assert all(len(set(snapshot)) > 1 for snapshot in jagged_snapshots)


def test_capacity_contrast_retains_both_outcome_classes_and_no_final_assets() -> None:
    replay_cases = tuple(
        load_calibration_redesign_v4_replay_case(_FIXTURE_ROOT, case_id) for case_id in _CASE_IDS
    )
    observed_acceptance = [
