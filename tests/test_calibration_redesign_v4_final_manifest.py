...
def _restore_pre_freeze_registry(root: Path) -> None:
    ...
    exclusions = [item for item in payload["explicit_exclusions"] if item not in obsolete]
    exclusions.extend(
        (
...
def test_final_manifest_loads_complete_verified_heldout_inventory() -> None:
    fixture_set = load_calibration_redesign_v4_final_manifested_fixture_set(_FIXTURE_ROOT)
...
def test_final_manifest_carry_forward_hashes_bind_active_stage_to_frozen_files() -> None:
    fixture_set = load_calibration_redesign_v4_final_manifested_fixture_set(_FIXTURE_ROOT)
    ...
    assert (
        ...
        == hashlib.sha256((_FIXTURE_ROOT / "final_evidence_index.json").read_bytes()).hexdigest()
    )
    assert (
...
def test_final_manifest_detects_one_byte_final_asset_change(tmp_path: Path) -> None:
    ...
    expected_code = CalibrationRedesignV4FinalManifestViolationCode.ASSET_INTEGRITY_MISMATCH
...
def test_final_manifest_detects_changed_index_bytes(tmp_path: Path) -> None:
    ...
    expected_code = CalibrationRedesignV4FinalManifestViolationCode.REGISTRY_PROVENANCE_MISMATCH
    assert error.value.code is expected_code
...
def test_final_manifest_freeze_is_deterministic_and_write_once(tmp_path: Path) -> None:
    ...
    expected_code = CalibrationRedesignV4FinalManifestViolationCode.DESTINATION_ALREADY_EXISTS
    assert error.value.code is expected_code
