"""Contract and loader tests for V5 calibration case pairs through V5-3e."""
...
_MIXED_CASE_IDS = tuple(f"CSV5-{number:03d}" for number in range(137, 149))
...
def test_loads_all_authorised_v5_calibration_case_pairs() -> None:
    ...
    mixed_cases = tuple(
        ...
        for case_id in _MIXED_CASE_IDS
    )

    replay_cases = (*curve_cases, *position_cases, *workload_cases, *mixed_cases)
    ...
    assert tuple(case.runtime_input.case_id for case in mixed_cases) == _MIXED_CASE_IDS
    ...
    assert {case.runtime_input.scenario_family_id for case in mixed_cases} == {
...
def test_runtime_assets_exclude_evaluation_only_outcome_fields() -> None:
    runtime_payload = json.loads(
        (_FIXTURE_ROOT / "inputs" / "cases" / "CSV5-143.json").read_text(encoding="utf-8")
    )
...
def test_workload_loader_rejects_a_misaligned_runtime_and_outcome_pair(
    ...
    outcome_path = root / "expected_outcomes" / "cases" / "CSV5-143.json"
    ...
    with pytest.raises(CalibrationSuccessorV5CaseContractError) as error:
        load_calibration_successor_v5_mixed_reliability_contrast_replay_case(root, "CSV5-143")
...
def test_loaders_reject_case_ids_outside_their_active_family_boundaries() -> None:
    ...
    with pytest.raises(CalibrationSuccessorV5CaseContractError) as mixed_error:
        load_calibration_successor_v5_mixed_reliability_contrast_replay_case(
            _FIXTURE_ROOT, "CSV5-201"
        )
    ...
    assert mixed_error.value.code is CalibrationSuccessorV5CaseViolationCode.CASE_ASSET_LAYOUT_ERROR
