"""Coverage and quarantine tests for the third V5 held-out workload-variation family."""
...
from specsafe.traces.calibration_successor_v5_final_cases import (
    ...
    load_calibration_successor_v5_final_workload_variation_replay_case,
    summarize_final_workload_variation_workloads,
)
...
_CASE_IDS = tuple(f"CSV5-{number:03d}" for number in range(219, 228))
...
def test_final_workload_variation_loads_exactly_nine_quarantined_heldout_case_pairs() -> None:
    replay_cases = tuple(
        load_calibration_successor_v5_final_workload_variation_replay_case(_FIXTURE_ROOT, case_id)
    ...
    assert all(
        case.runtime_input.scenario_family_id == "CSV5-FINAL-WORKLOAD-VARIATION"
...
def test_final_workload_variation_balances_workloads_and_retains_distinct_confidence_regions() -> (
    None
):
    replay_cases = tuple(
        load_calibration_successor_v5_final_workload_variation_replay_case(_FIXTURE_ROOT, case_id)
    ...
    for case in replay_cases:
        for context, outcome in zip(
            ...
            workload = context.workload_type.value
            confidence_by_workload[workload].append(context.conditional_survival_confidence)
    ...
    assert summarize_final_workload_variation_workloads(replay_cases) == {
        "code": 3,
        "open_ended_chat": 3,
        "structured_text": 3,
    }
    assert all(any(values) and not all(values) for values in outcomes_by_workload.values())
    mean_confidence_by_workload = {
        workload: sum(values) / len(values) for workload, values in confidence_by_workload.items()
    }
    assert (
        mean_confidence_by_workload["structured_text"]
        > mean_confidence_by_workload["code"]
        > mean_confidence_by_workload["open_ended_chat"]
    )


def test_final_workload_runtime_assets_exclude_posthoc_labels_and_tokens() -> None:
    runtime_payload = json.loads(
        (_FIXTURE_ROOT / "final_evaluation" / "inputs" / "cases" / "CSV5-225.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_final_workload_loader_rejects_later_family_reservation() -> None:
    with pytest.raises(CalibrationSuccessorV5FinalCaseContractError) as error:
        load_calibration_successor_v5_final_workload_variation_replay_case(
            _FIXTURE_ROOT, "CSV5-228"
        )

    assert error.value.code is CalibrationSuccessorV5FinalCaseViolationCode.CASE_ASSET_LAYOUT_ERROR


def test_final_workload_outcome_runtime_misalignment_is_rejected(tmp_path: Path) -> None:
    root = _copy_fixture_root(tmp_path)
    outcome_path = root / "final_evaluation" / "expected_outcomes" / "cases" / "CSV5-223.json"
    payload = json.loads(outcome_path.read_text(encoding="utf-8"))
    payload["fixture_id"] = "misaligned-final-workload-fixture"
    outcome_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")

    with pytest.raises(CalibrationSuccessorV5FinalCaseContractError) as error:
        load_calibration_successor_v5_final_workload_variation_replay_case(root, "CSV5-223")

    assert error.value.code is CalibrationSuccessorV5FinalCaseViolationCode.CASE_ALIGNMENT_ERROR


def test_final_workload_assets_do_not_change_frozen_calibration_manifest_or_fit_provenance() -> (
    None
):
