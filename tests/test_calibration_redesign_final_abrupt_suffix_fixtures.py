"""Regression tests for quarantined final abrupt-suffix fixture assets."""
...
FAMILY_ID = "CRV1-FINAL-ABRUPT-SUFFIX"
CASE_STEMS = (
    "CRV1-011-final-abrupt-suffix-code",
    "CRV1-012-final-abrupt-suffix-open-ended-chat",
)
FINAL_CASE_IDS = frozenset({"CRV1-011", "CRV1-012"})
...
def test_final_abrupt_suffix_pairs_load_under_quarantined_registry() -> None:
    ...
    for case_stem in CASE_STEMS:
        ...
        assert min(confidences[:2]) >= 0.70
        assert max(confidences[-2:]) <= 0.25
...
def test_fitted_temperature_artifact_excludes_abrupt_suffix_final_case_ids() -> None:
    artifact_path = (
        Path(__file__).resolve().parents[1]
        / "evidence"
        / "calibration"
