...
def test_v3_registry_loads_at_calibration_manifest_boundary() -> None:
    ...
    assert registry.v3_manifests_authored is True
    assert registry.next_authorized_artifact == "v3-quantile-isotonic-calibration-fit"
...
def test_v3_manifest_root_is_exact_and_prior_workload_guard_rejects_it() -> None:
...
def test_v3_registry_rejects_changed_calibration_case_budget(tmp_path: Path) -> None:
    fixture_root = _copy_manifest_root(tmp_path)
    registry_path = fixture_root / "scenario_family_registry.json"
    payload = json.loads(registry_path.read_text(encoding="utf-8"))
