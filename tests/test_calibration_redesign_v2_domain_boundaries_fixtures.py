...
def test_v2_domain_boundary_assets_preserve_open_interval_and_separated_labels() -> None:
    ...
    for case_id in DOMAIN_BOUNDARY_CASE_IDS:
        ...
        assert all(
            context.workload_type is WorkloadType.STRUCTURED_TEXT for context in runtime.contexts
        )
        ...
        confidences.extend(context.conditional_survival_confidence for context in runtime.contexts)
    ...
    assert any(math.isclose(confidence, 1.0 - CLIPPING_EPSILON) for confidence in confidences)
    assert any(confidence > 1.0 - CLIPPING_EPSILON for confidence in confidences)
