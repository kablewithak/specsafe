"""Coverage and quarantine tests for the second V5 held-out position-spread family."""
...
from specsafe.traces.calibration_successor_v5_final_cases import (
    ...
    load_calibration_successor_v5_final_position_spread_replay_case,
    summarize_final_position_spread_workloads,
)
...
_CASE_IDS = tuple(f"CSV5-{number:03d}" for number in range(210, 219))
...
def test_final_position_spread_loads_exactly_nine_quarantined_heldout_case_pairs() -> None:
    replay_cases = tuple(
        load_calibration_successor_v5_final_position_spread_replay_case(_FIXTURE_ROOT, case_id)
    ...
    assert all(
        case.runtime_input.scenario_family_id == "CSV5-FINAL-POSITION-SPREAD"
...
def test_final_position_spread_balances_workloads_and_position_confidence() -> None:
    replay_cases = tuple(
        load_calibration_successor_v5_final_position_spread_replay_case(_FIXTURE_ROOT, case_id)
        for case_id in _CASE_IDS
    )
    ...
    acceptance_by_position: dict[int, list[bool]] = defaultdict(list)
    confidence_by_position: dict[int, list[float]] = defaultdict(list)
    for case in replay_cases:
        for context, outcome in zip(
            case.runtime_input.contexts, case.expected_outcomes.outcomes, strict=True
        ):
            confidence_by_position[context.block_position_index].append(
                context.conditional_survival_confidence
            )
            acceptance_by_position[outcome.block_position_index].append(outcome.observed_acceptance)

    assert workloads == {"structured_text": 12, "code": 12, "open_ended_chat": 12}
    assert positions == Counter({1: 9, 2: 9, 3: 9, 4: 9})
    assert summarize_final_position_spread_workloads(replay_cases) == {
        "code": 3,
        "open_ended_chat": 3,
        "structured_text": 3,
    }
    assert all(any(values) and not all(values) for values in acceptance_by_position.values())
    mean_confidences = [
        sum(confidence_by_position[position]) / len(confidence_by_position[position])
        for position in range(1, 5)
    ]
    assert mean_confidences == sorted(mean_confidences, reverse=True)


def test_final_position_runtime_assets_exclude_posthoc_labels_and_tokens() -> None:
    runtime_payload = json.loads(
        (_FIXTURE_ROOT / "final_evaluation" / "inputs" / "cases" / "CSV5-214.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_final_position_loader_rejects_later_family_reservation() -> None:
    with pytest.raises(CalibrationSuccessorV5FinalCaseContractError) as error:
        load_calibration_successor_v5_final_position_spread_replay_case(_FIXTURE_ROOT, "CSV5-219")

    assert error.value.code is CalibrationSuccessorV5FinalCaseViolationCode.CASE_ASSET_LAYOUT_ERROR


def test_final_position_outcome_runtime_misalignment_is_rejected(tmp_path: Path) -> None:
    root = _copy_fixture_root(tmp_path)
    outcome_path = root / "final_evaluation" / "expected_outcomes" / "cases" / "CSV5-215.json"
    payload = json.loads(outcome_path.read_text(encoding="utf-8"))
    payload["fixture_id"] = "misaligned-final-position-fixture"
    outcome_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")

    with pytest.raises(CalibrationSuccessorV5FinalCaseContractError) as error:
        load_calibration_successor_v5_final_position_spread_replay_case(root, "CSV5-215")

    assert error.value.code is CalibrationSuccessorV5FinalCaseViolationCode.CASE_ALIGNMENT_ERROR


def test_final_position_assets_do_not_change_frozen_calibration_manifest_or_fit_provenance() -> (
    None
):
