...
CASE_IDS = tuple(f"CRV3-{number:03d}" for number in range(213, 219))


def test_final_saturated_capacity_cases_are_quarantined_balanced_and_unscored() -> None:
    ...
    for case_id in CASE_IDS:
        ...
        assert replay_case.runtime_input.scenario_family_id == "CRV3-FINAL-SATURATED-CAPACITY"
        ...
        assert all(
            context.capacity_snapshot.profile_id == "synthetic-v3-final-saturated-capacity"
...
def test_final_saturated_capacity_keeps_frozen_calibration_manifest_loadable() -> None:
...
def test_final_saturated_capacity_inputs_use_saturated_request_pressure() -> None:
    for case_id in CASE_IDS:
        replay_case = load_calibration_redesign_v3_final_evaluation_replay_case(
            FIXTURE_ROOT, case_id
        )
        assert all(
            8 <= context.capacity_snapshot.active_request_count <= 10
            for context in replay_case.runtime_input.contexts
        )
        assert [
