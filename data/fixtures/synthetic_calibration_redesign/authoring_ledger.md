...
status=registry-governed; calibration-corpus-frozen; calibration-artifact-fitted; final-mixed-reliability-authored; final-abrupt-suffix-authored; final-manifest-pending
...
| Keep all final case IDs outside the fitted artifact's case list. | The temperature scaler is calibration-only and remains frozen while final evidence is authored. | Blocks final-evaluation access and refitting. |
...
## Final abrupt-suffix authoring record
...
- The frozen temperature artifact must remain unchanged while final-evaluation evidence is authored.

## Next authoring gate

Both quarantined final families are now authored. Create a separate final-evaluation manifest and loader boundary before any held-out calibration assessment. The final manifest must verify only `final_evaluation` / `held_out_evaluation` assets and must not be accepted by any calibration fitting or temperature-search code path.
