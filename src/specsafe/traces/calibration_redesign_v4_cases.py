"""Typed V4 calibration curve-coverage case contracts and loaders.

Only the CRV4-101 through CRV4-112 calibration-only case pairs may be loaded at
this stage. Runtime inputs and post-hoc outcomes remain physically separate.
Final-evaluation and adversarial evidence remain absent, and this module does not
fit calibration or execute a scheduler.
"""
...
_V4_CURVE_COVERAGE_FAMILY_ID = "CRV4-CAL-CURVE-COVERAGE"
...
class CalibrationRedesignV4RuntimeInput(StrictContract):
    ...
    scenario_family_id: Literal["CRV4-CAL-CURVE-COVERAGE"]
    ...
    def validate_runtime_contexts(self) -> CalibrationRedesignV4RuntimeInput:
        ...
        for context in self.contexts:
            ...
            if context.decode_round != 0:
                raise ValueError(
                    "V4 curve-coverage calibration cases must use decode round zero"
                )
    ...
    def validate_outcomes(self) -> CalibrationRedesignV4ExpectedOutcomes:
        ...
        for outcome in self.outcomes:
            ...
            if outcome.decode_round != 0:
                raise ValueError(
                    "V4 curve-coverage calibration outcomes must use decode round zero"
                )
...
def validate_calibration_redesign_v4_replay_case_membership(
    ...
    """Verify that the pair belongs to the one currently authored V4 family."""
    ...
    if (
        family is None
        or family.scenario_family_id != _V4_CURVE_COVERAGE_FAMILY_ID
        or family.authoring_status != "calibration_curve_coverage_authored"
    ):
        raise CalibrationRedesignV4CaseContractError(
...
def load_calibration_redesign_v4_replay_case(
    ...
    """Load one authorised CRV4-101 through CRV4-112 calibration case pair."""
